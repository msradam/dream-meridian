...
OSM_TAGS = {
    'amenity': [
        ...
        # Emergency Services
        'police', 'fire_station', 'shelter', 'emergency_service',
        # Education (often evacuation centers)
        'school', 'kindergarten', 'college', 'university',
        # Community (emergency gathering points)
        'place_of_worship', 'community_centre', 'social_facility', 'refugee_site',
        # Government/Admin
        'townhall', 'courthouse', 'embassy', 'public_building',
        # Financial (cash access during emergencies)
        'bank', 'atm', 'money_transfer', 'mobile_money_agent',
        # Food/Water
        'marketplace', 'food_court', 'drinking_water', 'water_point', 'toilets',
        # Transport hubs
        'bus_station', 'ferry_terminal', 'fuel', 'parking',
        # Communication
        'post_office', 'telephone', 'internet_cafe',
    ],
    'healthcare': True,  # All healthcare facilities
    'emergency': True,   # Fire hydrants, assembly points, sirens
    'shop': [
        # Food
        'supermarket', 'convenience', 'grocery', 'general', 'food',
        'bakery', 'butcher', 'greengrocer', 'water',
        # Medical
        'pharmacy', 'chemist', 'medical_supply',
        # Hardware/Tools
    ...
    'building': [
        ...
        'mosque', 'temple', 'church', 'religious', 'cathedral',
        'fire_station', 'police',
        'warehouse',  # For aid storage
    ],
    'office': [
        'government', 'ngo', 'diplomatic', 'humanitarian',
        'un', 'international_organization',
        'telecommunication',
    ],
    'man_made': [
        'water_tower', 'water_well', 'water_works', 'pumping_station',
        'storage_tank', 'reservoir_covered',
    ...
    'aeroway': [
        'aerodrome', 'helipad', 'heliport',  # Emergency evacuation
    ],
    'railway': [
        'station', 'halt',
    ],
    ...
    'natural': [
        'spring', 'water',  # Natural water sources
    ],
    'landuse': [
...
def download_tiles(bounds, slug, zooms=TILE_ZOOM_LEVELS):
    ...
    for i, (z, x, y) in enumerate(all_tiles):
        ...
        if (i + 1) % 100 == 0 or (i + 1) == len(all_tiles):
            ...
            print(f"  [{i + 1}/{len(all_tiles)}] +{downloaded} cached:{skipped} failed:{failed} ETA:{remaining:.0f}s")
...
def export_nodes_to_duckdb(G_nx, db_path):
    ...
    nodes_data = [(node, data['y'], data['x'], data['x'], data['y']) 
                  for node, data in G_nx.nodes(data=True) if 'y' in data and 'x' in data]
...
def main():
    if len(sys.argv) < 3:
        ...
        print('  python build_location.py "Dhaka, Bangladesh" dhaka --tiles  # include offline tiles')
        sys.exit(1)
    ...
    tiles_info = f"static/tiles/{slug}/          (offline map tiles)" if download_tiles_flag else "(using online tiles)"
