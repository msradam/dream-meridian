...
def query(
    user_query: str, location: str = None, llm_url: str = LLAMA_URL
) -> QueryResult:
    ...
    if spatial_tools.G_nk is None:
        return QueryResult(
            ...
            modified_query=user_query,
            success=False,
            error="No location loaded. Use load_location() first.",
        )
    ...
    try:
        ...
        return QueryResult(
            ...
            success=False,
            error="Cannot connect to LLM server. Is llama-server running?",
        )
    ...
    try:
        ...
        return QueryResult(
            tool_name="",
            tool_args={},
            result={"raw_response": content if "content" in dir() else str(e)},
            geocoded=geocoded,
            query_time=time.time() - start_time,
            modified_query=modified_query,
            success=False,
...
def format_result_rich(result: QueryResult, location_info: dict = None) -> str:
    ...
    if result.geocoded:
        ...
        for place, info in result.geocoded.items():
            lines.append(
                f"  📍 {C.YELLOW}{place}{C.RESET} → {C.DIM}({info['lat']:.6f}, {info['lon']:.6f}){C.RESET}"
            )
        lines.append("")
    ...
    if "error" in data:
        ...
        lines.append(
            f"  Found {C.BOLD}{C.GREEN}{count}{C.RESET} {poi_type}(s) within {C.CYAN}{radius}m{C.RESET}"
        )
        pois = data.get("pois", [])
        for poi in pois[:6]:
        ...
        for poi in data.get("nearest_pois", [])[:5]:
            ...
            dist = poi.get("distance_m", 0)
            lines.append(
                f"    🚶 {C.BOLD}{name}{C.RESET} — {C.CYAN}{walk:.1f} min{C.RESET} {C.DIM}({dist:.0f}m){C.RESET}"
            )
    ...
    if result.llm_stats and result.llm_stats.tokens_per_sec > 0:
        ...
        lines.append(
            f"     Prompt:     {C.DIM}{stats.prompt_tokens} tokens ({stats.prompt_ms:.0f}ms){C.RESET}"
        )
        lines.append(
            f"     Completion: {C.DIM}{stats.completion_tokens} tokens ({stats.completion_ms:.0f}ms){C.RESET}"
        )
        lines.append(
            f"     Speed:      {C.CYAN}{stats.tokens_per_sec:.1f} tok/s{C.RESET}"
        )

    if location_info:
        lines.append(
            f"  🗺️  Graph: {C.DIM}{location_info.get('nodes', 0):,} nodes · {location_info.get('pois', 0):,} POIs{C.RESET}"
        )
...
def format_result_simple(result: QueryResult) -> str:
    ...
    if result.geocoded:
        places = ", ".join(
            f"{k} ({v['lat']:.4f}, {v['lon']:.4f})" for k, v in result.geocoded.items()
        )
    ...
    if "error" in data:
        ...
        lines.append(
            f"Reachable in {data['max_minutes']} min: {data['reachable_nodes']} nodes"
        )
...
Examples:
  ...
  %(prog)s --health
    ...
    parser.add_argument(
        "-l", "--location", help="Location slug (default: first available)"
    )
    parser.add_argument("--list", action="store_true", help="List available locations")
    parser.add_argument("--health", action="store_true", help="Check system health")
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Verbose output (include raw JSON)"
    )
    parser.add_argument("--json", action="store_true", help="Output raw JSON")
    parser.add_argument(
        "--simple", action="store_true", help="Simple output (no colors)"
    )
    ...
    if args.list:
        ...
        for slug, info in locations.items():
            ...
            print(
                f"    {C.DIM}{info['nodes']:,} nodes · {info['pois']:,} POIs · {info.get('places', 0):,} places{C.RESET}"
            )
        print()
        sys.exit(0)
    ...
    if args.health:
        ...
        if args.json:
            ...
            if health["locations"]:
                print(
                    f"               {C.DIM}{', '.join(health['locations'])}{C.RESET}"
                )
